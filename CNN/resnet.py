...
class Linear(Module):
    def __init__(self,size = ()):
        ...
        self.weight = Parameter(np.random.normal(0,np.sqrt(2 / size[0]),size=size),self.name+"weight")
        self.bias = Parameter(np.zeros((1,size[1])),self.name+"bias")
...
class Conv2d(Module):
    def __init__(self,initializer,out_channel,in_channel,kernel_size=(),stride = 1):
        ...
        self.stride = stride
        self.kernel = Parameter(initializer(np.empty((out_channel,in_channel,*kernel_size))),self.name+'kernel')
    ...
    def forward(self,X):
        ...
        self.bias = Parameter(np.zeros((self.out_channel,self.c_h*self.c_w)),self.name+'bias')
...
class BatchNorm2d(Module):
    def __init__(self, in_channel, eps=1e-05, momentum=0.5):
        ...
        self.gamma = Parameter(np.ones((in_channel, 1)),self.name+'gamma')
        self.beta = Parameter(np.zeros((in_channel, 1)),self.name+'beta')
        self.running_mean = 0
        self.running_var = 0
...
class L2SGD(Optimizer):
    ...
    def step(self):
        for param in self.params:
            if param.name == 'Linearweight' or param.name == 'Conv2dkernel':
...
class ResBlock(Module):
    def __init__(self,*args):
        self.layers = args
        self.names = names = [m.name for m in self.modules]
        super().__init__("ResBlock[\n\t"+"\n\t".join(names)+"\n]")

    def forward(self,X):
        resX = X
        for layer in self.layers:
            X = layer(X)
        if X.shape == resX.shape:
            return X + resX
        else:
            conv = Conv2d(OnesInitializer(),X.shape[1],resX.shape[1],(1,1))
            return X + conv(resX)[...,:X.shape[2],:X.shape[3]]

    def backward(self,G):
        resG = G
        for layer in self.layers[::-1]:
            G = layer.backward(G)
        if G.shape == resG.shape:
            return G + resG
        else:
            conv = Conv2d(OnesInitializer(), G.shape[1], resG.shape[1], (1, 1))
            G[...,:resG.shape[2],:resG.shape[3]] =  G[...,:resG.shape[2],:resG.shape[3]] + conv(resG)
            return G
...
if __name__ == '__main__':
    ...
    model = Model(
        ...
        MaxPooling(2, 2),
        ResBlock(
            Conv2d(kaiming, out_channel=9, in_channel=3, kernel_size=(3, 3)),  # 50*3*14*14 --> 50*9*12*12
            BatchNorm2d(9),
            ReLU(),
            Conv2d(kaiming, out_channel=27, in_channel=9, kernel_size=(3, 3)), # 50*9*12*12 --> 50*27*10*10
            BatchNorm2d(27),
        ),
        ReLU(),
        MaxPooling(2,2),
        Flatten(),
        Linear(size=((27*5*5),500)),
    ...
    for e in range(epoch):
        ...
        #print(params)
        lr = lrmax + 1 / 2 * (lrmax - lrmin) * (1 + np.cos( e / 10 * np.pi )) #余弦退火
        #opt = SGD(params,lr)
